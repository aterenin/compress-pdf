...
| Structural cleanup | Flate-compress uncompressed streams, deduplicate identical objects by content hash, drop unused resources (including fonts in an AcroForm's default resources that no default appearance string names, when the document has no XFA form) and unreferenced objects, renumber, raise the PDF version to what the output needs, write object streams and an xref stream. | whole file | `lopdf` (Rust) plus own dedupe | [~] |
