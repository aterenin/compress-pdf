...
fn encoding_dict_is_plain(doc: &Document, dict: &Dictionary) -> bool {
    ...
    match dict.get(b"Differences").map(|d| deref(doc, d)) {
        ...
        Ok(Object::Array(items)) => items.iter().all(|item| match item {
            ...
            Object::Name(n) => known_glyph_name(n),
            _ => false,
        }),
        Ok(_) => false,
    }
...
/// A glyph name a viewer's substitute for a standard font will have: the
/// names of the Standard, WinAnsi and MacRoman encodings, or a Unicode
/// form (`uni0041`, `u1F600`). Private names such as `g123` are not.
fn known_glyph_name(name: &[u8]) -> bool {
    ...
    KNOWN_GLYPHS.binary_search(&name).is_ok()
}

/// Sorted; the union of the three text encodings' glyph names.
const KNOWN_GLYPHS: [&str; 243] = [
    "A",
    "AE",
    "Aacute",
    "Acircumflex",
